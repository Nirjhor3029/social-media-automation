...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.whatsapp-queries.update", [$whatsappQuery->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('whstapp_subscriber') ? 'is-invalid' : '' }}"
                        ...
                        @foreach($whstapp_subscribers as $id => $entry)
                            <option value="{{ $id }}" {{ (old('whstapp_subscriber_id') ? old('whstapp_subscriber_id') : $whatsappQuery->whstapp_subscriber->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('question') ? 'is-invalid' : '' }}" type="text"
                        name="question" id="question" value="{{ old('question', $whatsappQuery->question) }}" required>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('answer') ? 'is-invalid' : '' }}" name="answer"
                        id="answer" required>{{ old('answer', $whatsappQuery->answer) }}</textarea>
