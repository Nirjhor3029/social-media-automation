...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.whatsappGroup.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.whatsapp-groups.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('whstapp_subscriber') ? 'is-invalid' : '' }}" name="whstapp_subscriber_id" id="whstapp_subscriber_id" required>
                    @foreach($whstapp_subscribers as $id => $entry)
                        <option value="{{ $id }}" {{ old('whstapp_subscriber_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
